...
AGENTMAIL_CFG = {
    ...
    'from_email': 'sender@agentmail.to',
...
def test_agentmail_save_requires_api_key(app_server, agentmail_mock):
    base = app_server['base_url']
    ...
    assert 'agentmail_api_key' in r.json()['error']


def test_agentmail_test_endpoint_provisions_and_sends(app_server, agentmail_mock):
    """First test send provisions an inbox + sends a probe message.
    Verifies both endpoints are hit and inbox_id is persisted."""
    base = app_server['base_url']
    _save_cfg(base)

    agentmail_mock.add(responses.POST, f'{AM_BASE}/inboxes',
                        json={'inbox_id': 'inb_abc123'}, status=201)
    agentmail_mock.add(responses.POST,
                        f'{AM_BASE}/inboxes/inb_abc123/messages/send',
    ...
    # The inbox_id should be persisted back into config so subsequent
    # sends reuse it.
    s = requests.get(f'{base}/api/integrations/email').json()
    assert s['agentmail_inbox_id'] == 'inb_abc123'

    # Verify auth header + payload shape on the send call
...
def test_agentmail_email_qr_enqueues_then_worker_sends(app_server, agentmail_mock):
    ...
    agentmail_mock.add(responses.POST, f'{AM_BASE}/inboxes',
                        json={'inbox_id': 'inb_xyz'}, status=201)
    agentmail_mock.add(responses.POST,
                        f'{AM_BASE}/inboxes/inb_xyz/messages/send',
                        json={'message_id': 'msg_1'}, status=200)
...
def test_agentmail_reuses_persisted_inbox_id(app_server, agentmail_mock):
    """Second send must NOT re-provision the inbox."""
    base = app_server['base_url']
    _save_cfg(base)

    # Pre-seed inbox_id directly in config to mimic a prior send.
    app = app_server['app']
    cfg = app.db.get_setting('email.config') or {}
    cfg['agentmail_inbox_id'] = 'inb_preexisting'
    app.db.set_setting('email.config', cfg)

    agentmail_mock.add(responses.POST,
                        f'{AM_BASE}/inboxes/inb_preexisting/messages/send',
                        json={'message_id': 'msg_1'}, status=200)
    ...
    # No POST /inboxes call - just the send.
